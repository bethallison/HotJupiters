...
input36=np.genfromtxt('HeatherCh2.txt', names=True, dtype=None, delimiter=' ')
...
xplanet36=[19,19,19,19,19,19,19,19,19,19,19,19,19,19,19,19,19,19,19,19,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12]

xplanet36=[18,18,18,18,18,18,18,18,18,18,18,18,18,18,18,18,18,18,18,18,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12,12]

xplanet36=[20,20,20,20,20,20,20,20,20,20,19,19,19,19,19,19,19,19,19,19,11,11,11,11,11,11,11,11,11,11,13,13,13,13,13,13,13,13,13,13]

xplanet36=[19,19,19,19,19,20,20,20,20,20,20,20,20,20,20,19,19,19,19,19,13,13,13,13,13,11,11,11,11,11,11,11,11,11,11,13,13,13,13,13]
...
for i in phase_values5:
    if flux_values36[counter]==-99: remove_positions.append(counter)
        ...
        if flux_values36[counter] < 0.001:
...
interpolation36=interp1d(new5x,comb36)
...
error36=0.00466/(1250375/len(phase_values36_removed))**0.5
...
res36=[]
res3=[]
for i in range(len(phase_values36_removed)-1):

    try: distance=np.abs(flux_values36_removed[i]-interpolation36(phase_values36_removed[i]))**2
    except Exception as e:
        print e
        print "Diagnostics: ", i, phase_values36_removed[i]
        break
...
for i in range(len(chi36)):
    distance36=np.abs(flux_values36_removed[i]-interpolation36(phase_values36_removed[i]))**2
    res36.append(distance36**0.5)
    distance236.append(flux_values36_removed[i]-interpolation36(phase_values36_removed[i]))
    res236.append(distance236[i])

print sum(chi36)
reserr36=[]
for i in range(len(res36)):
    reserr36.append(((2*chi36[i])**0.5)/len(phase_values36_removed))
...
axarr[0].set_title('4.5')
axarr[0].set_ylabel('Combined Flux in 4.5 microns')
...
axarr[1].set_ylim(-0.0003,0.0003)
axarr[1].set_xlabel('Longitude')
plt.savefig('modelextra.pdf')
plt.show()

print "4.5 Chi-Squared: ", np.sum(chi36)/len(phase_values36_removed), "+/-", np.sum(reserr36)
